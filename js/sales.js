...
async function openSaleModal(typeId) {
  ...
  /* تحميل العملاء النشطين للإكمال التلقائي */
  const customers = (await db.getAll('customers')).filter(c => c.status === 'active');
...
async function confirmSale() {
  ...
  if (isNew) {
    ...
    cData.id   = customerId;
    Sheets.backupCustomer(cData, t); /* غير انتظاري لعدم تأخير الواجهة */
  ...
  /* ── عميل موجود (تجديد) ── */
    ...
    /* تجديد الاشتراك */
    cust.subscriptionTypeId   = _activeSaleTypeId;
    cust.subscriptionTypeName = t.name;
    cust.startDate            = start;
    cust.endDate              = end;
    cust.status               = 'active';
    await db.update('customers', cust);
    Sheets.backupCustomer(cust, t);
  }
  ...
  showToast(`✅ تم تسجيل البيع — ربح ${fmtMoney(sale.profit)}`, 'success');
...
async function loadRecentSalesTable() {
  ...
  const sales = all.slice(-20).reverse();
  ...
  tbody.innerHTML = sales.map(s => `
    <tr>
      <td dir="ltr">${(s.date || '').slice(0, 16).replace('T', ' ')}</td>
